<?php
/**
 * Archive Template for Categories, Tags, and Taxonomies in KTD Store
 *
...
<!-- Archive Hero Banner (Full width with border-bottom) -->
<section class="ktd-blog-hero">
	<div class="ktd-container">
		...
		<div class="ktd-blog-hero-content">
			<span class="ktd-blog-hero-badge">Chuyên Mục Blog</span>
			<h1 class="ktd-blog-hero-title">
				<?php
				if ( is_category() ) {
					single_cat_title();
				} elseif ( is_tag() ) {
					single_tag_title( 'Thẻ: ' );
				} else {
					the_archive_title();
				}
				?>
			...
			<?php if ( get_the_archive_description() ) : ?>
				<div class="ktd-blog-hero-desc">
					<?php the_archive_description(); ?>
				</div>
			<?php else : ?>
				<p class="ktd-blog-hero-desc">
					Tổng hợp tất cả các bài viết, đánh giá và cẩm nang thuộc chủ đề này.
				</p>
			<?php endif; ?>
		</div>
	</div>
	...
	<div class="ktd-container">
		...
		<?php if ( ! empty( $categories ) ) : ?>
			<nav class="ktd-cat-filter-nav" aria-label="Lọc theo chuyên mục">
				<ul class="ktd-cat-filter-list">
					<li>
						<a href="<?php echo esc_url( get_permalink( get_option( 'page_for_posts' ) ) ?: home_url( '/blog/' ) ); ?>" class="ktd-cat-pill <?php echo 0 === $current_cat_id ? 'is-active' : ''; ?>">
							Tất cả bài viết
						</a>
					</li>
					<?php foreach ( $categories as $cat ) : ?>
						<li>
							<a href="<?php echo esc_url( get_category_link( $cat->term_id ) ); ?>" class="ktd-cat-pill <?php echo (int) $cat->term_id === $current_cat_id ? 'is-active' : ''; ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			<div class="ktd-blog-pagination">
				...
				the_posts_pagination( array(
					...
					'screen_reader_text' => 'Điều hướng phân trang chuyên mục',
				) );
				?>
			</div>
		...
		<?php else : ?>
			<div class="ktd-blog-empty">
				...
				<p>Hãy khám phá các chuyên mục khác hoặc quay lại trang tin tức chính.</p>
				<a href="<?php echo esc_url( get_permalink( get_option( 'page_for_posts' ) ) ?: home_url( '/blog/' ) ); ?>" class="ktd-btn ktd-btn-primary">Về Trang Tin Tức</a>
			</div>
		<?php endif; ?>
